...
int breeding(box_pattern ** populationPtr, int highestParent, int population_size, int x_max, int y_max,int num_particles){
    box_pattern * population = *populationPtr;
    int highest;
    ...
    copybox(&max_parent,&population[highestParent],num_particles);
    int i;
    ...
    for (i=0; i<population_size; i+=2){ //two children
        ...
        int parentOne=two;
        if (population[one].fitness > population[two].fitness) parentOne=one; //joust

        one = rand()%(population_size);
        two=rand()%(population_size);
        int parentTwo=two;
        if (population[one].fitness > population[two].fitness) parentTwo=one; //joust

        int splitPoint = rand() % num_particles; //split chromosome at point
        new_generation[i]= crossover(new_generation[i], population[parentOne], population[parentTwo], splitPoint,num_particles); //first child

        new_generation[i+1] = crossover(new_generation[i+1], population[parentTwo], population[parentOne], splitPoint,num_particles); //second child
    ...
    for (i=1; i<population_size; i++)
    {
        ...
        else if (new_generation[i].fitness>max_fitness)
        {
